...
class HierarchicalMesh(Mesh):
    ...
    def __init__(self, knots, dim):
        ...
        self.nlevels = 1
        self.aelem_level = {0: np.array((range(self.meshes[0].nelems)))}  # active elements on level
        self.delem_level = {0: np.array([])}  # deactivated elements on level
    ...
    def add_level(self):
        ...
        self.aelem_level[self.nlevels - 1] = (np.array([]))
        self.delem_level[self.nlevels - 1] = (np.array([]))
    ...
    def update_active_cells(self, marked_cells):
        ...
        for level in range(number_of_levels):
            if level in marked_cells:
                ...
                self.delem_level[level] = np.union1d(self.delem_level[level], marked_cells[level])

                new_cells[level + 1] = self.get_children(level, marked_cells[level])
                self.aelem_level[level + 1] = np.union1d(self.aelem_level[level + 1], new_cells[level + 1])
